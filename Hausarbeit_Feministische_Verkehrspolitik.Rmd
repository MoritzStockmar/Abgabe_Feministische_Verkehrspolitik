...
Hier wird das Wörterbuch für Mobilität getestet, indem alle Begriffe, auf die ein regülarer Ausdruck zutrifft und ihre Häufigkeit ausgegeben werden.
...
In diesem Abschnitt werden ein paar Weitere Ergebnisse, deren Visualisierung es nicht in die Arbeit geschafft haben, präsentiert. Dabei handelt es sich vor allem um Metavariablen- und Wörterbuchkonfigurationen, die nicht mehr unterbringbar waren.

### Beispielvisualisierung von veränderten Nutzungsmustern von Wörtern

```{r}
kfz_query <- c('"Kraftfahrzeug\\w+"', '"KFZ"')
auto_query <- c('"Auto"')

freqs_kfz <- polmineR::dispersion(gparl, query = kfz_query, s_attribute = "year", cqb = TRUE, freq = TRUE)

freqs_auto <- polmineR::dispersion(gparl, query = auto_query, s_attribute = "year", cqb = TRUE, freq = TRUE)

kfz_vs_auto <- left_join(freqs_kfz, freqs_auto, by = "year")

kfz_vs_auto %>%
  count(year, count.x, count.y) %>%
  pivot_longer(cols = starts_with("count")) %>%
  ggplot(aes(x = year, y = value, group = name, color = name)) +
    geom_line() + 
    labs(
      title = "Verlauf der Begriffsnutzung von Auto und KFZ",
      x = "Jahr",
      y = "Anzahl Nennungen",
      color = "Wort",
    ) +
  scale_color_manual(labels = c('KFZ', 'Auto'), values = c("red", "blue"))
